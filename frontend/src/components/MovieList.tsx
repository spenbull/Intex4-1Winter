  ...
  const getGenreList = (movie: Movie): string => {
    ...
    return Object.entries(movie)
      ...
      .map(([key]) => {
        ...
        return parts
          .map((word) =>
            alwaysUpper.has(word.toUpperCase())
              ? word.toUpperCase()
    ...
    const fetchAverageRating = async () => {
      ...
      try {
        const res = await fetch(
          `https://cinenichegroup0401-backend-affvedfvhnhyc4fp.eastus-01.azurewebsites.net/api/Movie/ratings/average/${selectedMovie.show_id}`,
        );
        const data = await res.json();
